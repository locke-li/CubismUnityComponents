...
namespace Live2D.Cubism.Rendering
{
    ...
    public sealed class CubismUGUIRenderController : MaskableGraphic, ICubismUpdatable
    {
        /// <summary>
        /// Model opacity.
        /// </summary>
        ...
        /// </summary>
        [NonSerialized]
        private ICubismUGUIDrawOrderHandler _drawOrderHandlerInterface;

        /// <summary>
        /// Listener for draw order changes.
        /// </summary>
        private ICubismUGUIDrawOrderHandler DrawOrderHandlerInterface
        {
            get
            {
                if (_drawOrderHandlerInterface == null)
                {
                    _drawOrderHandlerInterface = DrawOrderHandler.GetInterface<ICubismUGUIDrawOrderHandler>();
                }
        ...
        public Object OpacityHandler
        {
            ...
            set { _opacityHandler = value.ToNullUnlessImplementsInterface<ICubismUGUIOpacityHandler>(); }
        }
        ...
        /// <see cref="OpacityHandler"/> backing field.
        /// </summary>
        private ICubismUGUIOpacityHandler _opacityHandlerInterface;

        /// <summary>
        /// Listener for opacity changes.
        /// </summary>
        private ICubismUGUIOpacityHandler OpacityHandlerInterface
        {
            get
            {
                if (_opacityHandlerInterface == null)
                {
                    _opacityHandlerInterface = OpacityHandler.GetInterface<ICubismUGUIOpacityHandler>();
                }
        ...
        private Texture mainTextureFromRenderer;
        public override Texture mainTexture => mainTextureFromRenderer??s_WhiteTexture;
        ...
        private void FetchMainTextureFromRenderers()
        {
            foreach(var renderer in Renderers) {
                if(renderer.MainTexture != null) {
                    mainTextureFromRenderer = renderer.MainTexture;
                    break;
                }
            }
        }
        ...
        #region UGUI graphics

        private List<Vector3> vertexBuffer = new List<Vector3>(256);
        private List<int> triangleBuffer = new List<int>(512);
        private List<Color32> colorBuffer = new List<Color32>(256);
        private List<Vector2> uvBuffer = new List<Vector2>(256);

        protected override void OnPopulateMesh(VertexHelper vh)
        {
            vh.Clear();
            int offset = 0;
            foreach(var renderer in Renderers.OrderBy(r => r.MeshRenderer.sortingOrder))
            {
                if(!renderer.Visible) continue;
                if(renderer.IsHitDrawable) continue;
                ...
                if(colorBuffer.Count == 0) {
                    for(int o = 0; o < vertexBuffer.Count; ++o) {
                        vh.AddVert(vertexBuffer[o] * pixelPerUnit, Color.white, uvBuffer[o]);
                    }
                }
                else {
                    for(int o = 0; o < vertexBuffer.Count; ++o) {
                        vh.AddVert(vertexBuffer[o] * pixelPerUnit, colorBuffer[o], uvBuffer[o]);
                    }
                }
                for(int t = 0; t < triangleBuffer.Count; t+=3) {
                    vh.AddTriangle(triangleBuffer[t]+offset, triangleBuffer[t+2]+offset, triangleBuffer[t+1]+offset);
                }
